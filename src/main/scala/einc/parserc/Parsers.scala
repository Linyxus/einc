...
object Parsers:
  ...
  extension (s: String) def parseWith[X](p: Parser[X]): ParseOutcome[X] =
    p.run(ParseInput(s, SourcePos.init))(using ParseContext.empty)

  /** A parser that consumes a character if it matches the predicate */
  def predicate(pred: Char => Boolean, errMsg: String | Null = null): Parser[Char] = Parser: input =>
    input.currentChar match
      case None => ParseError("unexpected end of input", input.current, Nil)
      case Some(ch) =>
        ...
        else ParseError(msg, input.current, Nil)

  /** A parser that asserts EOF */
  def eof: Parser[Unit] = Parser: input =>
    input.currentChar match
      case None => ParseOk((), input, Nil)
      case Some(ch) => ParseError(s"expected EOF, but found more input", input.current, Nil)
  ...
  def string(s: String): Parser[Unit] = Parser: input =>
    if input.source.substring(input.current.pos).startsWith(s) then
      ...
      ParseError(s"expected string `$s`", input.current, Nil)
